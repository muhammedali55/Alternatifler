...
(function($) {
	$.fn.styleSwitcher1 = function(options){		
		var defaults = {	
		...
		if (opts.useCookie && opts.manageCookieLoad) {
			...
			if(isCookie){
				...
				$("link[id=styless]").attr("href",newStyle);
				baseStyle = newStyle;
			}
			else{
		...
		var baseStyle = $("link[id=styless]").attr("href");
		if(opts.preview){
			$(opts.container + " a").click(
				...
				function () {
					$("link[id=styless]").attr("href",baseStyle);
				}
			);
		}
		...
		$(opts.container + " a").click(
			function () {
				$("#stylesw1").find("a").removeClass("current-item");
